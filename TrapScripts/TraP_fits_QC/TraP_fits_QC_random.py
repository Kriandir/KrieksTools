...
import glob
import getRMS
import tools_kriek as tools


hdu=0
if len(sys.argv) != 6:
    print 'python TraP_fits_QC.py <sigmaClip> <sigmaRej> <f> <frequencies> <struniquetodeepimg or n if no deepimg>'
    exit()
...
allFreqs=sys.argv[4]
deepcheck = sys.argv[5]

freqs=[]
imageData=[]
for filename in glob.glob('*image-pb.fits'):
    if  deepcheck != 'n' and deepcheck in filename:
        deep = filename
    else:
...
#   we do this selection because we accept all images with a better then average rms
    if image[2]<thresholds['all'][0]+thresholds['all'][1]:
