...
# if you feed boot = True via function call in TraP_fits_QC_random you will force bootstrapping algorithm
    if boot == True:
        print 'forcing bootstrap because invalid solution'
...
def bootstraps(data,sigma,xlabel,pltname,freq):
    ...
    print "bootstrapsteps: " +str(steps)
    databoot= []
...
#
    ...
    plsq = so.leastsq(res, p, args = (hist_x[0], range_x)) # fit Gaussian to data
    fit2 = plsq[0][2]*norm2(range_x, plsq[0][0], plsq[0][1]) # create Gaussian distribution for plotting on graph
    if plsq[1] > 4: # In case leastsq doesn't converge to a solution initially, bootstrap additional data
        return fit_hist(x,sigma,name,filename,freq,boot=True)
